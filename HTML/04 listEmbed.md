...
- Ordered List(순서가 있는 목록, 정렬)
  ...
  `<li>` 에 value 값을 넣어줄 경우 지정한 값부터 차례대로 정렬됨.
...
- 웹에서 사용하는 이미지 유형
  ...
  | --------------------- | --------------- | -------------------------------------- | ------------------------------------------------------------ |
  | JPEG(사용해본 확장자) | `image/jpeg`    | `.jpg` `.jpeg` `.jfif` `.pjpeg` `.pjp` | 정지 이미지의 손실 압축에 적합 (현재 가장 많이 사용)         |
  | PNG(사용해본 확장자)  | `image/png`     | `.png`                                 | PNG는 원본 이미지를 보다 정확하게 보여주거나 **투명도**가 필요한 경우 JPEG보다 선호 |
  | GIF(사용해본 확장자)  | `image/gif`     | `.gif`                                 | 여러장의 이미지로 이루어진 애니메이션 표현                   |
  | WEBP                  | `image/webp`    | `.webp`                                | 구글이 만든 이미지 포맷, 품질, 압축률 등이 우수하지만 지원 브라우저가 제한적 |
  | SVG(사용해본 확장자)  | `image/svg+xml` | `.svg`                                 | 다양한 크기로 정확하게 그려야 하는 아이콘, 다이어그램 등에 사용 |
